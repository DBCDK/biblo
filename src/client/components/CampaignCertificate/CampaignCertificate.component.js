...
export class CampaignCertificate extends Component {
      ...
      <div>
        ...
        <div id="pageHeader">
          ...
          <span className="profile-name">
            <span dangerouslySetInnerHTML={{__html: sanitizeHtml(profile.displayName)}} /> fra {branchShortName}
          </span>
        </div>
        <div id="pageFooter">
    ...
    return (
      <div className="frontpage">
        <div className="frontpage--header">
          ...
          <div>
            {dimplomaText} {campaign.campaignName}
          </div>
      ...
      return (
        <div key={`post_${post.id}`} className="contribution">
          ...
          <div className="post--header">
            <h3>
              Indlæg i <span className="group--title" dangerouslySetInnerHTML={{__html: sanitizeHtml(group.name)}} />
            </h3>
            <div className="post--date">{dateString}</div>
          </div>

          <div className="post--content">{this.renderContributionContent(post)}</div>
    ...
    return reviews.map((review, key) => {
      ...
      return (
        <div key={`review_${review.id}`} className="contribution">
          ...
          <div className="post--content">{this.renderContributionContent(review)}</div>
          <div className="campaign-logo">
            <img src={campaign.logos.medium} />
          </div>
        </div>
  ...
  render() {
    ...
    const branchShortName = (Array.isArray(library.branchShortName)
      ? library.branchShortName[0]
    ...
    return (
      <div>
        ...
        <div className="group-contributions">{this.renderGroupContributions(contributions, campaign, profile)}</div>

        <div className="review-contributions">{this.renderReviewContributions(contributions, campaign, works)}</div>
      </div>
    );
  }
}
