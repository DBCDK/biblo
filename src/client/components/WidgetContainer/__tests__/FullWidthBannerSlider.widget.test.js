...
describe('Test FullWidthBannerSlider Widget', () => {
  it('Test empty FullWidthBannerSlider widget can render inside a WidgetContainer', () => {
    ...
    expect(
      $root
        .find('.widget--title--generic')
        .first()
        .text()
    ).to.equal('empty slider test');
  });
  ...
  it('Test FullWidthBannerSlider widget can render with one image', () => {
    ...
    expect(
      $root
        ...
        .first()
        .text()
    ).to.equal('this is a dummy banner title');
    expect(
      $root
        .find('.full-width-banner--image--text--container > p')
        .first()
        .text()
    ).to.equal('This is a dummy banner description!');
    expect(
      $root
        .find('.fwbs--dots')
        .first()
  ...
  it('Test FullWidthBannerSlider widget can render with two images', () => {
    ...
    expect(
      wrapper
        ...
        .first()
        .text()
    ).to.equal('And yet another dummy title');
    expect(
      wrapper
        .find('.full-width-banner--image--text--container > p')
        .first()
        .text()
    ).to.equal('This is the second dummy banner description!');
